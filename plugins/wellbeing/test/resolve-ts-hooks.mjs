...
// resolve, but that plain node (type stripping, no transpile) does not. Here
// the local imports name a `.ts` file by its compiled `.js` extension
// (`../server/presence.js`), which node looks for literally and does not find.
//
...
// stripping, which leaves no transpiler to get wrong. `client/surface.test.tsx`
// still needs a JSX transform and stays on the already-declared vitest.
...
export async function resolve(specifier, context, next) {
  try {
    ...
    // baffling ERR_UNKNOWN_FILE_EXTENSION. The one JSX test is on vitest instead.
    for (const ext of [".ts"]) {
