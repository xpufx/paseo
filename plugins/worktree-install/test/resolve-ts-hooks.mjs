...
//
// This covers the shared and server suites only. The two client suites are
// deliberately not run through here: they render `client/fleet-view.tsx` and
// friends for real, and node's type stripping does not transform JSX, so those
// need a JSX-capable runner (see package.json test script and #609).
//
